...
void ConnectionManager::update_value(std::string_view key, std::string_view value) {
    ...
    try {
      ...
      if (request.get_method() != HTTPMethod::GET) {
        ...
        if (target_rel_path == "/get/values") {
          ...
          strncpy(buf, m_values_csv.c_str(), m_values_csv.length());
          len = m_values_csv.length();
        }
        else {
